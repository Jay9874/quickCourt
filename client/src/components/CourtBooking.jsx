...
const CourtBooking = () => {
  ...
  async function loadVenue () {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_SERVER_URL}/api/venues/${venueId}`,
        {
          withCredentials: true
        }
      )
      console.log('the res from is : ', res.data)
      setVenue(res.data)
      setSport(res.data.courts[0].sport)
      setDataLoaded(true)
      // Calculate total price based on selected sport
      const selectedSportData = res.data.sports[sport]
      const totalPrice = selectedSportData
        ? selectedSportData.pricePerHour * duration * selectedCourts.length
        : 0
  ...
  useEffect(() => {
    ...
    console.log('selected courts are: ', selectedCourts)
    selectedCourts.map(court => {
      console.log('court: ', court)
      newPrice += court.price * duration
    })

    setTotalPrice(newPrice)
  }, [duration, selectedCourts])
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      const booking = {
        ...
        startTime,
        duration,
        courts: selectedCourts.map(item => item._id)
      }
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-gray-600 mt-2'>
                Book your preferred court and time slot
              </p>
            </div>
        ...
        <div className='bg-white rounded-lg shadow-md p-6'>
          <form className='space-y-6'>
            ...
            <div>
              ...
              >
                {venue.courts.map(item => (
                  <option key={item._id} value={item.sport}>
                    {item.sport}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              <div className='relative'>
                ...
                >
                  <span
                    className={startTime ? 'text-gray-900' : 'text-gray-500'}
                  >
            ...
            <div>
              ...
              <div className='relative'>
                ...
                {showCourtPicker && venue && (
                  <div className='absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-md shadow-lg z-10 max-h-48 overflow-y-auto'>
                    {venue.courts
                      .filter(court => {
                        console.log('the court is: ', court)
                        console.log('the sport is: ', sport)
                        return court.sport === sport
                      })
                      .map((court, index) => (
                        <button
                          key={index}
                          ...
                          onClick={() => {
                            if (
                              !selectedCourts.find(c => c._id === court._id)
                            ) {
                              setSelectedCourts(prev => [...prev, court])
                            }
                ...
                <div className='mt-3 flex flex-wrap gap-2'>
                  {selectedCourts.map(court => (
                    ...
                    >
                      ...
                      <button
                        type='button'
                        onClick={() => handleRemoveCourt(court._id)}
            ...
            <div className='bg-gray-50 p-4 rounded-lg'>
              ...
              {/* <p className='text-sm text-gray-600 mt-1'>
                {venue &&
                  `₹${selectedSportData.pricePerHour}/hour × ${duration} hour(s) × ${selectedCourts.length} court(s)`}
              </p> */}
            </div>
